...
def test_create_ticket_success(client: TestClient, db, test_event):
    data = {
...
def test_create_ticket_invalid_event_id(client: TestClient):
    data = {
        "event_id": 9999,
        "seat_num": "C1",
        "price": 25.00
...
def test_update_ticket_success_admin(client_with_superuser: TestClient, db, test_ticket):
...
def test_update_ticket_success_organizer(client_with_organizer: TestClient, db, test_ticket):
    updated_seat_num = "D1"
...
def test_update_ticket_unauthorized_visitor(client_with_visitor: TestClient, test_ticket):
    data = {"price": 10.00}
...
def test_cancel_ticket_success(client: TestClient, db, test_ticket):
...
def test_cancel_ticket_missing(client: TestClient):
