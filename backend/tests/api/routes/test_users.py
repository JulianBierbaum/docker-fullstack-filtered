...
def test_register_user_duplicate_email(client_with_superuser: TestClient, test_visitor):
    ...
    assert f"Email '{test_visitor.email}' already exists in the db." in response.json()["detail"]
...
def test_get_user_success(client: TestClient, test_visitor):
    response = client.get(f"/api/users/{test_visitor.id}")
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["id"] == test_visitor.id
    assert response.json()["email"] == test_visitor.email

def test_get_user_missing(client: TestClient):
...
def test_update_user_success_admin(client_with_superuser: TestClient, db, test_visitor):
    ...
    response = client_with_superuser.put(f"/api/users/update/{test_visitor.id}", json=data)
    assert response.status_code == status.HTTP_200_OK
...
def test_update_user_duplicate_email(client_with_superuser: TestClient, test_visitor, test_organizer):
    data = {"email": test_organizer.email}
    response = client_with_superuser.put(f"/api/users/update/{test_visitor.id}", json=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert f"Email '{test_organizer.email}' already exists in the db." in response.json()["detail"]

def test_update_user_unauthorized_visitor(client_with_visitor: TestClient, test_visitor):
    data = {"username": "Unauthorized Update"}
    response = client_with_visitor.put(f"/api/users/update/{test_visitor.id}", json=data)
    assert response.status_code == status.HTTP_403_FORBIDDEN
    assert "The user doesn't have enough privileges" in response.json()["detail"]
