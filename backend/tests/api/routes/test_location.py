...
def test_create_location_success_admin(client_with_superuser: TestClient, db):
    data = {"name": "Admin Created Location", "address": "Admin Street 1"}
...
def test_create_location_unauthorized_visitor(client_with_visitor: TestClient):
    data = {"name": "Visitor Attempt Location", "address": "Visitor Street 1"}
...
def test_create_location_duplicate_name(
    client_with_superuser: TestClient, test_location
):
    data = {"name": test_location.name, "address": "Duplicate Street 1"}
    response = client_with_superuser.post("/api/locations/", json=data)
...
def test_update_location_success_admin(
    client_with_superuser: TestClient, db, test_location
):
    updated_name = "Updated Location Name"
    data = {"name": updated_name}
    response = client_with_superuser.put(
        f"/api/locations/{test_location.id}", json=data
    )
    assert response.status_code == status.HTTP_200_OK
...
def test_update_location_unauthorized_visitor(
    client_with_visitor: TestClient, test_location
):
...
def test_delete_location_success_admin(
    client_with_superuser: TestClient, db, test_location
):
...
def test_delete_location_unauthorized_visitor(
    client_with_visitor: TestClient, test_location
):
